/**
 * @File: fermat_spiral_edge_generator_test.cpp
 * @Date: February 2022
...
// Testing class
class FermatSpiralEdgeGeneratorTest : public ::testing::Test
{
public:
  FermatSpiralEdgeGeneratorTest()
...
TEST_F(FermatSpiralEdgeGeneratorTest, Constructor)
{
  rrt::edge::FermatSpiralEdgeGeneratord edge_gen(0.01, 2);
}

TEST_F(FermatSpiralEdgeGeneratorTest, MakeEdge3Point)
{
  for(size_t curv_it = 0; curv_it < this->number_curvature_tests; ++curv_it)
  {
  for(size_t res_it = 0; res_it < this->number_res_tests; ++res_it)
  {
    rrt::edge::FermatSpiralEdgeGeneratord edge_gen(this->resolutions_to_test[res_it], this->curvature_to_test[curv_it]);
        ...
        if((point_2[2] != point_3[2]) and edge_gen.valid(point_1, point_2, point_3))
        {
          ...
          for(Eigen::Index point_it = 1; point_it < test_edge.rows(); ++point_it)
          {
            if((test_edge.row(point_it-1).template leftCols<2>()-test_edge.row(point_it).template leftCols<2>()).norm() > 1e-6)
            {
              ...
              EXPECT_NEAR(((test_edge.row(point_it-1).leftCols<2>().array() + pointing_vects.row(point_it-1).array()) - test_edge.leftCols<2>().row(point_it).array()).matrix().norm(), 0, this->resolutions_to_test[res_it] * double(10));
            }
          }
...
TEST_F(FermatSpiralEdgeGeneratorTest, MakeEdge2Point)
{
  for(size_t res_it = 0; res_it < this->number_res_tests; ++res_it)
  {
    rrt::edge::FermatSpiralEdgeGeneratord edge_gen(this->resolutions_to_test[res_it], 1);
...
TEST_F(FermatSpiralEdgeGeneratorTest, SetOrientation)
{
  rrt::edge::FermatSpiralEdgeGeneratord edge_gen(0.01, 1);
...
TEST_F(FermatSpiralEdgeGeneratorTest, Valid)
{
  for(size_t curv_it = 0; curv_it < this->number_curvature_tests; ++curv_it)
  {
    rrt::edge::FermatSpiralEdgeGeneratord edge_gen(1000, this->curvature_to_test[curv_it]);
...
TEST_F(FermatSpiralEdgeGeneratorTest, CurveDistance)
{
  for(size_t curv_it = 0; curv_it < this->number_curvature_tests; ++curv_it)
  {
    rrt::edge::FermatSpiralEdgeGeneratord edge_gen(200, this->curvature_to_test[curv_it]);
...
TEST_F(FermatSpiralEdgeGeneratorTest, GettersSetters)
{
  rrt::edge::FermatSpiralEdgeGeneratord edge_gen(0, 1);
