/**
 * @File: arc_fillet_edge_generator_test.cpp
 * @Date: October 2021
...
// Testing class
class ArcFilletEdgeGeneratorTest : public ::testing::Test
{
public:
  ArcFilletEdgeGeneratorTest()
...
TEST_F(ArcFilletEdgeGeneratorTest, Constructor)
{
  rrt::edge::ArcFilletEdgeGeneratord edge_gen(0.01, 2);
}

TEST_F(ArcFilletEdgeGeneratorTest, MakeEdge3Point)
{
  for(size_t curv_it = 0; curv_it < this->number_curvature_tests; ++curv_it)
  {
    for(size_t res_it = 0; res_it < this->number_res_tests; ++res_it)
    {
      rrt::edge::ArcFilletEdgeGeneratord edge_gen(this->resolutions_to_test[res_it], double(1)/this->curvature_to_test[curv_it]);
...
TEST_F(ArcFilletEdgeGeneratorTest, MakeEdge2Point)
{
  for(size_t res_it = 0; res_it < this->number_res_tests; ++res_it)
  {
    rrt::edge::ArcFilletEdgeGeneratord edge_gen(this->resolutions_to_test[res_it], 0);
            ...
            for(point_2[1] = -100; point_2[1] < 100.1; point_2[1] += 20)
            {
              for(point_3[0] = -100; point_3[0] < 100.1; point_3[0] += 20)
              {
                for(point_3[1] = -100; point_3[1] < 100.1; point_3[1] += 20)
                {
                  ...
                  if(edge_gen.makeEdge(point_1, point_2, point_3, test_edge))
                  {
                    double calc_dist = 0;
