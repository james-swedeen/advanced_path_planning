...
int main(int argc, char** argv)
{
  ...
  rclcpp::Node::SharedPtr node = std::make_shared<rclcpp::Node>("dubins_path_planning_demo_node");

  /// RRT Tools
  rrt::search::BatchTools<3> rrt_tools;
  ...
  const double edge_resolution = node->get_parameter("edge_resolution").as_double();
  ...
  // Setup Cost Function
  rrt_tools.cost_function = std::make_shared<rrt::cost::DistanceCostFunction<3,
                                                                             true,
                                                                             rrt::edge::DubinsPathEdgeGeneratord::DistanceFunc>>(
                              edge_resolution, double(1)/max_curvature);
  ...
  rrt_tools.nn_searcher = std::make_shared<rrt::tree::kdt::NearestNeighborSearcher<3,rrt::edge::DubinsPathEdgeGeneratord::DistanceFunc>>(
  ...
  rrt_tools.steering_function =
    std::make_shared<rrt::steer::ConstSteeringFunction21d>(near_radius,
                                                           near_radius,
  ...
  rrt_tools.edge_generator = std::make_shared<rrt::edge::DubinsPathEdgeGeneratord>(
                               edge_resolution,
                               double(1)/max_curvature);
  ...
  /// Solve the problem
  rrt::search::SolutionPtr3d solution = rrt::search::bitSearch<3,
                                                               rrt::search::RRTVersions::PRUNE_SUB_OPTIMAL_NODES,
                                                               //rrt::search::RRTVersions::NULL_VERSION,
                                                               double,
                                                               Eigen::RowMajor>(rrt_tools);
  const Eigen::Matrix<double,Eigen::Dynamic,3,Eigen::RowMajor> solution_path = solution->generatePath(rrt_tools.edge_generator);

  // Check that the path is valid
  {
    rrt::edge::EdgeGeneratorPtr<3> fine_edge_gen =
      std::make_shared<rrt::edge::DubinsPathEdgeGeneratord>(double(0.001)*occupancy_grid->resolution(),double(1)/max_curvature);

    Eigen::Matrix<double,Eigen::Dynamic,3,Eigen::RowMajor> fine_sol = solution->generatePath(fine_edge_gen);
  ...
  std::for_each(std::next(tree_logger->buffer.cbegin()),
                ...
                [](const Eigen::Matrix<double,Eigen::Dynamic,3,Eigen::RowMajor>& edge)
                {
