 ...
 *
 * @brief
 * Function used to find a path between two points using RRT based algorithms.
 **/

#ifndef RRT_SEARCH_SEARCH_FUNCTIONS_RRT_SEARCH_HPP
#define RRT_SEARCH_SEARCH_FUNCTIONS_RRT_SEARCH_HPP
 ...
 *
 * @brief
 * Uses a RRT based algorithms to find a path from the start to the end.
 *
 * @templates
...
SolutionPtr<DIM,SCALAR,OPTIONS> rrtSearch(Tools<DIM,SCALAR,OPTIONS>& tools)
{
      ...
      ++output->iterations)
  {
    ...
    std::list<Connection<DIM,SCALAR,OPTIONS>> potential_edges;
    ...
    if constexpr(rewireOperation(VERSION) or reconnectOperation(VERSION))
    {
      if(!(conditionalActivationRewire(VERSION) and best_path.empty()))
      {
        ...
        else // If we want to rewire but never got the neighborhood set
        {
          ...
          //neighbors.reserve(new_point_neighbors_size);
        ...
        rewire<DIM,VERSION,SCALAR,OPTIONS>(new_node,
                                           tree,
                                           neighbors,
                                           tools.edge_generator,
                                           tools.obstacle_checker,
    ...
    if((not best_path.empty()) and (output->cost != best_path.back()->cgetCost()))
    {
      ...
      if constexpr(smart(VERSION))
      {
        ...
        optimizePath<DIM,VERSION,SCALAR,OPTIONS>(best_path,
                                                 tree,
                                                 tools.edge_generator,
                                                 tools.obstacle_checker,
                                                 tools.cost_function);
